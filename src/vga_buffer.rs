...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(C)] // guarantees that the struct's fields are laid out exactly like in C
struct ScreenChar {
    ascii_character: u8,
...
impl Writer {
    pub fn write_byte(&mut self, byte: u8) {
        match byte {
            ...
            byte => {
                ...
                self.column_position += 1; // move to next column
            }
        }
    }
    ...
    fn clear_row(&mut self, row: usize) {
        ...
        for col in 0..BUFFER_WIDTH {
            self.buffer.chars[row][col].write(blank); // write blank to every column
        }
    }
}
